...
def make_numpy_dataset(n: int = N_ITEMS, dim: int = DIM, seed: int = 0) -> tuple[np.ndarray, NumpyDataset]:
    rng = np.random.default_rng(seed)
    data = rng.standard_normal((n, dim)).astype(np.float32)
    return data, NumpyDataset(data)


def make_tensor_dataset(n: int = N_ITEMS, dim: int = DIM, seed: int = 1) -> tuple[torch.Tensor, TensorDataset]:
...
def test_rqkmeans() -> bool:
    ...
    results.append(check("reconstruction is non-trivial", bool(recon.abs().max() > 1e-6)))
